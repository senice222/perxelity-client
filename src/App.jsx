        ...
        <div className="bg-white/90 dark:bg-gray-800 shadow-lg p-6 fade-in">
          <div className="mb-6">
            <div>
              {response.reasoningMode && response.thinking && showThinking && (
                <div className="mb-6 bg-gray-100 p-4 rounded-lg">
                  <h2 className="text-lg font-semibold mb-2 text-black">🧠 Ход рассуждений:</h2>
                  <div className="text-gray-700">
                    {processCitations(response.thinking, response.citations).split('\n').map((paragraph, idx) => (
            ...
            <div className="text-gray-700 dark:text-gray-300 markdown-content">
              <ReactMarkdown
                ...
                components={{
                  ...
                  a({ href, children, ...props }) {
                    ...
                    const isSourceLink = /\[\d+\]/.test(childrenText) && href?.startsWith('#citation-');

                    // Если это внутренняя ссылка на источник
                    if (isSourceLink) {
                      return (
                        <a
                          href={href}
              ...
              >
                {processCitations(response.answer, response.citations)}
              </ReactMarkdown>
            </div>
          </div>
          ...
          {/* {response.reasoningMode && response.reasoningSteps?.length > 0 && showThinking && (
            <div className="mb-6">
              <h2 className="text-lg font-semibold mb-2 text-black">🧠 Reasoning Steps:</h2>
                  ...
                  <li key={index} className="reasoning-step">
                    ...
                    <span className="text-gray-700 dark:text-gray-300">
                      <div>
                        ...
                        >
                          {processCitations(step, response.citations)}
                        </ReactMarkdown>
                      </div>
                    </span>
                  </li>
            ...
            <div>
              ...
              <ul className="list-none">
                {response.citations.map((citation, index) => (
                  <li key={index} className="mb-2" id={`citation-${index + 1}`}>
                    <a
                      href={citation}
                      target="_blank"
                      rel="noopener noreferrer"
...
// Функция для преобразования числовых цитат в markdown-ссылки на внешние источники
const processCitations = (text, citations) => {
  if (!citations || citations.length === 0) return text;

  // Заменяем цитаты [число] на markdown-ссылки на соответствующие внешние источники
  return text.replace(/\[(\d+)\]/g, (match, num) => {
    ...
    return match; // Если источника с таким номером нет, оставляем как есть
  });
};
